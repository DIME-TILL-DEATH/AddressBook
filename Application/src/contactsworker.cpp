    ...
    : m_clientManager {ClientManager::instance()}
{
    ...
    connect(&m_clientManager, &ClientManager::connectionStateChanged,
            this, &ContactsWorker::onConnectionStateChanged);
    connect(&m_clientManager, &ClientManager::contactsResponce,
            this, &ContactsWorker::onContactsDownloadSuccessed);
}
...
//  так понятнее:  net::Package package{QVariant::fromValue(true), net::PackageType::CONTACTS_REQUEST};
...
void ContactsWorker::onContactsDownloadSuccessed(const std::vector<QVariant> &data)
{
