...
void ServerManager::onDataArrived()
{
    ...
    forever
    {
        ...
        // но правильнее не сразу обрабатывать пакеты, а добавлять их в очередь и разбирать
        // уже worker'ом
    }
}
...
void ServerManager::handlePackage(net::Package &package, QTcpSocket *socket)
{
    ...
    switch(package.type())
    {
        ...
        {
            m_worker.handleContactsRequest(socket);
            break;
        }
        default:
        {
...
void ServerManager::connectSignals()
{
    ...
    connect(&m_worker, &ContactsWorker::contactsRequestCompleted,
